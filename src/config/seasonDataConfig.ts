...
const TEAM_DOMESTIC_LEAGUE: Record<string, DomesticLeagueKey> =
  Object.fromEntries([
...
export function getDomesticLeagueKeyForTeam(
  teamId: string,
): DomesticLeagueKey | null {
...
/** Labels for team IDs. */
export const TEAM_LABELS: Record<string, string> = Object.fromEntries(
...
export const LEAGUES_WITH_TEAMS: {
  leagueName: string;
  teams: { id: TeamSlug; label: string }[];
}[] = [
  { leagueName: "La Liga", teams: LA_LIGA_TEAMS.map((id) => TEAM_BY_ID[id]) },
  {
    leagueName: "Premier League",
    teams: PREMIER_LEAGUE_TEAMS.map((id) => TEAM_BY_ID[id]),
  },
  {
    leagueName: "Bundesliga",
    teams: BUNDESLIGA_TEAMS.map((id) => TEAM_BY_ID[id]),
  },
...
export const TEAM_CLUB_COMPETITIONS: Record<
  string,
  { id: ClubCompetitionId; label: string }[]
> = Object.fromEntries([
...
export const TEAM_CLUB_TROPHIES: Record<string, ClubTrophyId[]> =
  Object.fromEntries([
...
/** Labels for club trophy ids, including legacy aliases (e.g. usc-trophy → same as uesc-trophy). */
export const CLUB_TROPHY_LABEL: Record<string, string> = {
  ...Object.fromEntries(CLUB_TROPHIES.map((t) => [t.id, t.label])),
  "usc-trophy": "UEFA Super Cup",
};
...
export const COMPETITION_LABELS: Record<string, string> = {
  ...
  /** Legacy club competition id (alias of uesc); keep labels for existing saves */
  usc: "UEFA Super Cup",
};
...
export const INT_TROPHIES: { id: IntTrophyId; label: string }[] = [
  ...
  { id: "world-cup", label: "FIFA World Cup" },
...
/** Trophy cabinet rows use trophy ids (e.g. ucl-trophy); season rows use competition ids (e.g. ucl). */
export const TROPHY_ID_TO_LABEL: Record<string, string> = {
  ...CLUB_TROPHY_LABEL,
  ...Object.fromEntries(INT_TROPHIES.map((t) => [t.id, t.label])),
};

export function labelForCompetitionOrTrophyId(id: string): string {
  const t = String(id ?? "").trim();
  if (!t) return t;
  return TROPHY_ID_TO_LABEL[t] ?? COMPETITION_LABELS[t] ?? t;
}
...
const DOMESTIC_AWARD_GROUPS: Record<DomesticLeagueKey, AwardSelectGroup[]> = {
  ...
  "ligue-1": [
    ...
    },
    {
      label: "French cups",
      awards: [
        "Coupe de France Best Player",
        "Trophée des Champions Best Player",
      ],
    },
  ],
};
...
export function getAwardSelectGroupsForTeam(
  teamId: string,
): AwardSelectGroup[] {
...
export function isSelectableAwardForTeam(
  awardName: string,
  teamId: string,
): boolean {
...
/** Matches selectable award name "Man of the match" (case-insensitive). */
export function isManOfTheMatchAward(awardName: string): boolean {
  return awardName.trim().toLowerCase() === "man of the match";
}
...
export function awardUsesQuantity(awardName: string): boolean {
  ...
  if (!s) return false;
  if (isManOfTheMatchAward(awardName)) return true;
